...
**Service**: `app/synthesis/services/qwen-potential-client.ts`
Qwen analyzes each window to extract:
...
**Service**: `app/synthesis/services/graph-service.ts`
For each topic-context pair, Qwen generates:
...
Edit `app/synthesis/services/qwen-potential-client.ts`:
```typescript
