...
| **Qwen (Alibaba)** | Recommended | Generates personalized conversation strategies |
...
# Recommended: Alibaba Qwen (strategy generation)
QWEN_API_KEY=...
...
3. **Conversation strategies** are generated using Qwen to personalize interactions
...
2. **Extraction**: Qwen extracts potential topics, contexts, and items
...
- **Alibaba Qwen** for strategy generation and analysis
...
- **API Costs**: Running studies with many participants can incur significant costs. Monitor usage in your Anthropic, Alibaba, and OpenAI dashboards
...
- **User-Provided API Keys**: For public demos, set `NEXT_PUBLIC_REQUIRE_USER_API_KEYS=true` to have users provide their own Qwen keys
</details>
...
**Demo Mode** — Users provide their own Qwen API keys:
\`\`\`bash
...
- The Anthropic, Alibaba, and OpenAI teams for API access
